   ...
   <form class="popUpForm" ACTION="registerWorkshop.do" METHOD="POST">
      <!-- Complaint form starts -->
      	...
      	<span class="InputFormLineGroup">
	     	...
	     	<input type="text" name="email" id="email"class="floatLeft adjustWidthPWFemail <%=Helper.validate("email") %>"  placeholder="Email: sample@sample.com" value="<%=Helper.getStoredString("email") %>" class=" <%=Helper.validate("email") %>">
		...
		<input type="hidden" value="<%=request.getParameter("registration")%>" name="registration">
     	<span class="twoInputButtonsWrapper adjustRightMarginPWFbtn">
      		<input type="reset" onClick="closeResponseBox()" class="tButton redGlow" value="Cancel">
      		<input type="submit" onClick="" value="Register" class="tButton greenGlow">  
      	</span>
